...
def search_documents(keyword: str) -> dict[str, Any]:
    """Search conservation documents by keyword.

    Lists all objects in the bucket, downloads text content, and performs
    case-insensitive substring matching. PDF files are skipped during
    content search.

    Args:
        keyword: The search term to look for in document content.

    Returns:
        A dict with 'results' (list of matching document metadata with
        excerpts) and 'count'.
    """
    s3 = _get_s3_client()
